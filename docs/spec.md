...
- **判定フロー**:
  ...
  2. 測位の可用性チェック: `accuracyMeters == null || 非有限 || > gpsAccuracyBadMeters`、または緯度経度が非有限・範囲外
     ...
     - OUTER 状態の場合: 位置が内側に見えても警告を解除せず OUTER を維持し、距離情報は最後に信頼できた測位の値を使う。精度良好なfixだけが `inner`/`near` に復帰できる
     - このとき `StateSnapshot.navigationFromLastReliableFix` を true にする。案内が現在位置ではないという事実は判定した状態機械が持ち、表示側は再計算しない。表示側で精度としきい値から求め直すと、「使えない測位」の条件が増えたとき（座標がNaNで精度だけ良好なfixなど）に食い違い、古い案内を現在位置として表示してしまう
  3. 精度良好の場合: エリア内/外を判定
...
- **途絶警告の反復**: 途絶が続くあいだ既定60秒間隔で警告を出し直し、本文に経過時間を含める。自動再接続を打ち切った後は「再接続しています」とは書かず、停止した旨とアプリで確認するよう促す。書き続けると来ない復旧を待たせることになる。1回だけの通知では、無音通知と1回の短い振動を見逃した時点で監視の停止に気づけなくなるため。fix復帰・監視停止で反復を止める。
...
- **発報失敗の扱い**: OUTER発報で失敗した経路があれば、失敗した経路名を含めて画面に表示する。ログだけに残すと、警報音が鳴っていないのに通常のOUTER画面が出て「警報は動いている」と誤解される。1分ミュートからの復帰失敗、アプリ復帰時の鳴らし直し失敗も同様に表示する。
- **警告の表示先**: 発報・停止の失敗は `alertReliabilityWarning` として、利用者が閉じるまで消えない常設バナーに出す。4秒で消えるSnackbarでは、走行中に数十秒後へ画面を見る利用者には届かない。監視開始時の消音確認を通過した時点で、前セッションの警告は現状を表していないので消す。
