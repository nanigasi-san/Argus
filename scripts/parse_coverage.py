...
def parse_lcov(filepath):
    ...
    for record in records:
        ...
        if sf_match and lf_match and lh_match:
            ...
            files.append(
                {"file": filepath, "total": total, "hit": hit, "coverage": coverage}
            )
    ...
    overall_coverage = (
        round((hit_lines / total_lines * 100), 1) if total_lines > 0 else 0
    )
...
if __name__ == "__main__":
    ...
    for file in stats["files"]:
        print(
            f"  {file['file']}: {file['coverage']}% ({file['hit']}/{file['total']} lines)"
        )
